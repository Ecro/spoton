...
전통 프로그래밍:
  규칙(코드) + 데이터 → 결과
  예: if (accel > 30g) → "충격 감지"  ← 사람이 규칙 작성

머신러닝:
  데이터 + 결과(라벨) → 규칙(모델)을 자동으로 학습
  예: (진동 패턴, 위치) × 300발 → SVR 모델이 패턴 학습
      → 새로운 진동 패턴 입력 → "여기에 맞았을 것" 추정

핵심: "사람이 규칙을 못 만들 때" ML을 쓴다
  → 39개 진동 특징 → 위치를 규칙으로 표현하기 불가능
  → 하지만 데이터에서 패턴은 존재 → ML이 찾아줌
```
...
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  지도학습 (Supervised Learning) ← SpotOn이 사용하는 방식     │
│    입력: (특징, 정답) 쌍                                     │
│    예: (진동 패턴, 실제 위치) × 300발                        │
│    모델이 "입력 → 정답" 매핑을 학습                          │
│    → 새 입력이 오면 정답을 추정                              │
...
│    입력: 특징만 (정답 없음)                                   │
│    "비슷한 것끼리 묶어라" (클러스터링)                        │
│    → 라벨링 불필요하지만 "어디"인지 직접 알 수 없음           │
│                                                             │
│  강화학습 (Reinforcement Learning)                           │
│    행동 → 환경 피드백(보상) → 학습                           │
│    게임, 로봇 제어용 → SpotOn에는 적합하지 않음              │
│                                                             │
│  자기지도학습 (Self-supervised Learning)                     │
│    대량 비라벨 데이터로 "표현"을 학습 → 소량 라벨로 fine-tune │
│    → Phase 1 이후 확장 가능성 (Part 6 참조)                  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### 0.3 회귀 (Regression) vs 분류 (Classification)

```
회귀: 연속적인 숫자를 예측
  입력: 진동 특징 39개
  출력: x = 3.2cm, y = -1.5cm  ← 연속값
  → SpotOn의 위치 추정 (SVR)

분류: 카테고리를 예측
  입력: 스윙 특징 30개
  출력: "포핸드" (5개 중 1개)  ← 이산값
  → SpotOn의 샷 분류 (Random Forest)
```

### 0.4 SVR (Support Vector Regression) — 쉽게 설명

```
핵심 아이디어:
  "데이터 점들 사이에 가장 넓은 '도로'를 깔아라"

1차원 예시 (직선):
  데이터: (특징1, 위치) = (2.0, 3cm), (4.0, 7cm), (3.0, 5cm), ...
  SVR은 이 점들을 관통하는 선을 찾되,
  ε(엡실론) 만큼의 "도로 폭" 안에 있는 점은 무시 (허용 오차)
  ...
  도로 밖에 있는 점 = "서포트 벡터" (모델을 결정하는 핵심 데이터)
  → 모델 크기 = 서포트 벡터 개수 × 특징 수 (보통 ~100-200개)

고차원 (39개 특징):
  39차원 공간에서 "초평면 도로"를 깔기
  → 사람은 상상 못 하지만 수학적으로는 동일

RBF 커널의 마법:
  직선으로 안 되는 복잡한 패턴 → "데이터를 고차원으로 변환"
  변환된 공간에서는 직선으로 분리 가능!
  ...
  RBF(Radial Basis Function):
    K(x, x') = exp(-γ × ||x - x'||²)
    = "두 점이 가까우면 1, 멀면 0" (가우시안 종 모양)

하이퍼파라미터:
  C = "오차를 얼마나 엄격히 처벌할까"
    작으면: 넓은 도로, 많은 오차 허용 (과소적합)
    크면: 좁은 도로, 오차 최소화 (과적합 위험)

  γ (gamma) = "RBF 커널의 영향 범위"
    작으면: 넓은 범위 (부드러운 예측)
    크면: 좁은 범위 (구불구불한 예측, 과적합 위험)

  ε (epsilon) = "허용 오차 폭"
    0.5cm이면: ±0.5cm 이내 오차는 무시
    → SpotOn에서 0.5cm = 적당한 허용치

SVR의 장점 (SpotOn 관점):
  ✓ 적은 데이터로 잘 동작 (300발이면 충분할 수 있음)
  ✓ 과적합에 강함 (ε + 정규화)
  ✓ 모델 크기 작음 (서포트 벡터만 저장)
  ✓ 추론 빠름 (~100µs)
  ✓ SmartDampener 논문에서 검증 (3.03cm)
```

### 0.5 Random Forest — 쉽게 설명

```
핵심 아이디어:
  "여러 전문가(결정 트리)에게 물어보고 다수결"
...
Random Forest = 20개 트리의 다수결:
  트리 1: "포핸드"  ← 약간 다른 데이터로 학습
  트리 2: "포핸드"  ← 약간 다른 특징 사용
  트리 3: "백핸드"
  트리 4: "포핸드"
  ...
  투표: 포핸드 14표, 백핸드 4표, 서브 2표
  → 결과: "포핸드" (다수결 승!)

왜 잘 되나:
  → 각 트리는 "데이터의 일부"와 "특징의 일부"만 봄
  → 개별 트리는 부정확하지만, 합치면 정확해짐 (앙상블)
  → "바보 전문가 20명 > 천재 1명" 원리

하이퍼파라미터:
  n_estimators = 20  → 트리 개수 (많을수록 정확, MCU 메모리 고려)
  max_depth = 8      → 트리 깊이 (깊을수록 복잡, 과적합 위험)

  → 트리 20개 × 깊이 8 = ~5KB Flash (MCU에 충분)

Random Forest의 장점:
  ✓ 학습 빠름 (수 초)
  ✓ 해석 가능 (어떤 특징이 중요한지 알 수 있음)
  ✓ 과적합에 강함 (앙상블)
  ✓ 분류 문제에 매우 효과적
```

### 0.6 Neural Network (신경망) — 비교용
...
구조:
  입력층 (39개 특징) → 은닉층 (64개 뉴런) → 은닉층 (32개) → 출력 (x, y)

  각 뉴런: output = activation(Σ(weight × input) + bias)
  activation: ReLU, sigmoid, tanh 등

  학습 = weight와 bias를 조정하여 오차 최소화
  방법: 역전파(backpropagation) + 경사하강법(gradient descent)
...
CNN (Convolutional Neural Network):
  "특징 추출 자동화" ← 핵심 차이!
  입력: raw 신호 (312 samples × 9 axes) — 특징 추출 불필요
  1D Conv → Pool → 1D Conv → Pool → Dense → 출력
  → 더 높은 정확도 가능 (2~3cm?)
  → 하지만 Flash ~50KB+, 학습 데이터 1,000+발 필요
  → TFLite Micro 필요

Transformer:
  "주의 메커니즘" — 시계열 내 중요한 부분에 집중
  → 최고 정확도 가능하지만 MCU에는 너무 큼 (수 MB)
  → SpotOn에는 비현실적

SpotOn 업그레이드 경로:
  SVR (현재) → MLP (정확도 부족시) → CNN (최후 수단)
  ↓
  300발       600발                 1,000+발
  11KB Flash  20KB Flash            50KB+ Flash
  200µs       500µs                 5ms+
```

### 0.7 과적합 (Overfitting) vs 과소적합 (Underfitting)

```
과적합: "시험지를 외웠지만 응용을 못 함"
  학습 데이터: MAE 1cm (완벽!)
  새 데이터:   MAE 8cm (엉망!)
  → 모델이 학습 데이터의 노이즈까지 외워버림

과소적합: "너무 단순해서 패턴을 못 찾음"
  학습 데이터: MAE 7cm
  새 데이터:   MAE 7cm (일관되게 나쁨)
  → 모델이 너무 단순하거나, 특징이 부족

적정 학습 (Good fit):
  학습 데이터: MAE 3cm
  새 데이터:   MAE 4cm (약간 더 나쁘지만 비슷)
  → 이것이 목표!

확인 방법: 교차 검증 (Cross-validation)
  데이터를 5등분:
  [1][2][3][4][5]

  1차: [1]로 테스트, [2][3][4][5]로 학습
  2차: [2]로 테스트, [1][3][4][5]로 학습
  3차: [3]로 테스트, ...
  4차: [4]로 테스트, ...
  5차: [5]로 테스트, ...

  → 5번의 테스트 MAE 평균 = 실제 성능 추정치
  → 이것이 "CV MAE" (Cross-Validated Mean Absolute Error)

방지법:
  과적합 → 데이터 추가, 정규화 강화 (C 줄이기), 특징 줄이기
  과소적합 → 모델 복잡도 증가, 특징 추가, C 키우기
```

### 0.8 특징 추출 (Feature Engineering) — 왜 중요한가

```
raw 신호 vs 특징:

raw: 312 samples × 9 axes = 2,808개 숫자
  → SVR에 직접 넣으면? → 차원이 너무 높아 학습 어려움, 과적합

특징: 39개 숫자 (raw에서 의미 추출)
  → 차원 축소: 2,808 → 39 (72배!)
  → 각 숫자가 물리적 의미를 가짐 (감쇠율, 주파수 에너지 등)
  → SVR이 39개로 충분히 학습 가능

비유:
  raw = "이 사람의 얼굴 사진 (1,000,000 픽셀)"
  특징 = "키 180cm, 눈 갈색, 코 높음" (10개 특징)
  → 특징만으로 사람 구분 가능!

SpotOn에서:
  raw = "진동 파형 전체"
  특징 = "피크 80g, 감쇠율 0.3, 146Hz 에너지 높음" (39개)
  → 특징만으로 위치 추정 가능!

좋은 특징의 조건:
  1. 위치에 따라 값이 변함 (discriminative)
  2. 충격 강도에 무관 (force-irrelevant) — Chen et al. 발견
  3. 노이즈에 안정적 (robust)
  4. 계산 가능 (MCU에서 실시간)
```
